...
void EquilibrationScaling::RegisterOptions(const SmartPtr<RegisteredOptions> & /*roptions*/
)
{
}

bool EquilibrationScaling::InitializeImpl(const OptionsList &options, const std::string &prefix)
{
...
void EquilibrationScaling::DetermineScalingParametersImpl(
    const SmartPtr<const VectorSpace> x_space, const SmartPtr<const VectorSpace> c_space,
    const SmartPtr<const VectorSpace> d_space, const SmartPtr<const MatrixSpace> jac_c_space,
    const SmartPtr<const MatrixSpace> jac_d_space, const SmartPtr<const SymMatrixSpace> /*h_space*/, const Matrix &Px_L,
    const Vector &x_L, const Matrix &Px_U, const Vector &x_U, Number &df, SmartPtr<Vector> &dx, SmartPtr<Vector> &dc,
    SmartPtr<Vector> &dd)
{
    DBG_ASSERT(IsValid(nlp_));

    SmartPtr<Vector> x0 = x_space->MakeNew();
    if (!nlp_->GetStartingPoint(GetRawPtr(x0), true, NULL, false, NULL, false, NULL, false, NULL, false)) {
    ...
    for (Index ieval = 0; ieval < num_evals; ieval++) {
        ...
        while (!done) {
            ...
            done = (nlp_->Eval_grad_f(*xpert, *grad_f) && nlp_->Eval_jac_c(*xpert, *jac_c) &&
                    nlp_->Eval_jac_d(*xpert, *jac_d));
            if (!done) {
                Jnlst().Printf(
                    J_WARNING, J_INITIALIZATION,
...
PointPerturber::PointPerturber(const Vector &x0, Number random_pert_radius, const Matrix &Px_L, const Vector &x_L,
