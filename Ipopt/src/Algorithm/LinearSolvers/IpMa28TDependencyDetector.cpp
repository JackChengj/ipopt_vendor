...
extern "C" {
void F77_FUNC(ma28part, MA28PART)(ipindex *TASK, ipindex *N, ipindex *M, ipindex *NZ, ipnumber *A, ipindex *IROW,
                                  ipindex *ICOL, ipnumber *PIVTOL, ipindex *FILLFACT, ipindex *IVAR, ipindex *NDEGEN,
                                  ipindex *IDEGEN, ipindex *LIW, ipindex *IW, ipindex *LRW, ipnumber *RW, ipindex *IERR);
}
...
void Ma28TDependencyDetector::RegisterOptions(SmartPtr<RegisteredOptions> roptions)
{
    roptions->AddBoundedNumberOption("ma28_pivtol", "Pivot tolerance for linear solver MA28.", 0.0, true, 1., false, 0.01);
}

bool Ma28TDependencyDetector::InitializeImpl(const OptionsList &options, const std::string &prefix)
{
    options.GetNumericValue("ma28_pivtol", ma28_pivtol_, prefix);
    return true;
}

bool Ma28TDependencyDetector::DetermineDependentRows(Index n_rows, Index n_cols, Index n_jac_nz, Number *jac_c_vals,
                                                     Index *jac_c_iRow, Index *jac_c_jCol, std::list<Index> &c_deps)
{
    DBG_START_METH("Ma28TDependencyDetector::DetermineDependentRows", dbg_verbosity);
