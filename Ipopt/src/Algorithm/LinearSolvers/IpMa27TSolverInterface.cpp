...
Ma27TSolverInterface::Ma27TSolverInterface(SmartPtr<LibraryLoader> hslloader_) :
...
{
    roptions->AddBoundedIntegerOption("ma27_print_level", "Debug printing level for the linear solver MA27", 0, 4, 0,
                                      "0: no printing; 1: Error messages only; 2: Error and warning messages; 3: Error "
                                      "and warning messages and terse monitoring; 4: All information.");
    ...
    roptions->AddBoundedNumberOption(
        "ma27_pivtolmax", "Maximum pivot tolerance for the linear solver MA27.", 0.0, true, 1.0, true, 1e-4,
        "Ipopt may increase pivtol as high as ma27_pivtolmax to get a more accurate solution to the linear system.");
    roptions->AddLowerBoundedNumberOption(
        "ma27_liw_init_factor", "Integer workspace memory for MA27.", 1.0, false, 5.0,
    ...
    roptions->AddLowerBoundedNumberOption(
        "ma27_la_init_factor", "Real workspace memory for MA27.", 1.0, false, 5.0,
        "The initial real workspace memory = la_init_factor * memory required by unfactored system. "
        "Ipopt will increase the workspace size by ma27_meminc_factor if required.");
    roptions->AddLowerBoundedNumberOption(
        "ma27_meminc_factor", "Increment factor for workspace size for MA27.", 1.0, false, 2.0,
        "If the integer or real workspace is not large enough, Ipopt will increase its size by this factor.");
    roptions->AddBoolOption("ma27_skip_inertia_check", "Whether to always pretend that inertia is correct.", false,
                            "Setting this option to \"yes\" essentially disables inertia check. "
                            "This option makes the algorithm non-robust and easily fail, but it might give some "
                            "insight into the necessity of inertia control.",
                            true);
    roptions->AddBoolOption(
        "ma27_ignore_singularity",
        "Whether to use MA27's ability to solve a linear system even if the matrix is singular.", false,
        ...
        "In some cases this might be better than using Ipopt's heuristic of small perturbation of the lower diagonal "
        "of the KKT matrix.",
        true);
}

void Ma27TSolverInterface::SetFunctions(IPOPT_DECL_MA27A(*ma27a), IPOPT_DECL_MA27B(*ma27b), IPOPT_DECL_MA27C(*ma27c),
...
bool Ma27TSolverInterface::InitializeImpl(const OptionsList &options, const std::string &prefix)
{
    if (user_ma27a != NULL) {
    ...
    if (!warm_start_same_structure_) {
        ...
        ASSERT_EXCEPTION(dim_ > 0 && nonzeros_ > 0, INVALID_WARMSTART,
                         "Ma27TSolverInterface called with warm_start_same_structure, but the problem is solved for "
                         "the first time.");
    }
...
ESymSolverStatus Ma27TSolverInterface::MultiSolve(bool new_matrix, const Index *airn, const Index *ajcn, Index nrhs,
...
ESymSolverStatus Ma27TSolverInterface::InitializeStructure(Index dim, Index nonzeros, const Index *airn, const Index *ajcn)
{
    ...
    if (!warm_start_same_structure_) {
        ...
        ASSERT_EXCEPTION(
            dim_ == dim && nonzeros_ == nonzeros, INVALID_WARMSTART,
...
ESymSolverStatus Ma27TSolverInterface::SymbolicFactorization(const Index *airn, const Index *ajcn)
{
    ...
    if (Jnlst().ProduceOutput(J_MOREMATRIX, J_LINEAR_ALGEBRA)) {
        Jnlst().Printf(J_MOREMATRIX, J_LINEAR_ALGEBRA,
                       "\nMatrix structure given to MA27 with dimension %" IPOPT_INDEX_FORMAT
                       " and %" IPOPT_INDEX_FORMAT " nonzero entries:\n",
                       dim_, nonzeros_);
    ...
    if (iflag != 0) {
        ...
        if (iflag == 1)
            Jnlst().Printf(J_ERROR, J_LINEAR_ALGEBRA,
                           "The index of a matrix is out of range.\nPlease check your implementation of the Jacobian "
    ...
    try {
        ...
        Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA, "Setting integer work space size to %" IPOPT_INDEX_FORMAT "\n", liw_);
        iw_ = new Index[liw_];
        ...
        Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA, "Setting double work space size to %" IPOPT_INDEX_FORMAT "\n", la_);
        a_ = new Number[la_];
    } catch (const std::bad_alloc &) {
...
ESymSolverStatus Ma27TSolverInterface::Factorization(const Index *airn, const Index *ajcn, bool check_NegEVals,
                                                     Index numberOfNegEVals)
{
    ...
    if (la_increase_) {
        ...
        Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA,
                       "In Ma27TSolverInterface::Factorization: Increasing la from %" IPOPT_INDEX_FORMAT
                       " to %" IPOPT_INDEX_FORMAT "\n",
                       la_old, la_);
        try {
            a_ = new Number[la_];
        } catch (const std::bad_alloc &) {
    ...
    if (liw_increase_) {
        ...
        Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA,
                       "In Ma27TSolverInterface::Factorization: Increasing liw from %" IPOPT_INDEX_FORMAT
                       " to %" IPOPT_INDEX_FORMAT "\n",
                       liw_old, liw_);
        try {
            iw_ = new Index[liw_];
    ...
    if (iflag == -3 || iflag == -4) {
        ...
        Jnlst().Printf(J_WARNING, J_LINEAR_ALGEBRA,
                       "MA27BD returned iflag=%" IPOPT_INDEX_FORMAT
                       " and requires more memory.\n Increase liw from %" IPOPT_INDEX_FORMAT " to %" IPOPT_INDEX_FORMAT
                       " and la from %" IPOPT_INDEX_FORMAT " to %" IPOPT_INDEX_FORMAT " and factorize again.\n",
                       iflag, liw_old, liw_, la_old, la_);
        try {
            iw_ = new Index[liw_];
            a_ = new Number[la_];
        } catch (const std::bad_alloc &) {
            Jnlst().Printf(J_STRONGWARNING, J_LINEAR_ALGEBRA,
                           "Failed to allocate more working space (iw_ and a_) for MA27\n");
            throw;  // will be caught in IpIpoptApplication
        }
        if (HaveIpData()) {
    ...
    if (iflag == -5 || (!ignore_singularity_ && iflag == 3)) {
        ...
        Jnlst().Printf(J_WARNING, J_LINEAR_ALGEBRA,
                       "MA27BD returned iflag=%" IPOPT_INDEX_FORMAT
                       " and detected rank deficiency of degree %" IPOPT_INDEX_FORMAT ".\n",
                       iflag, missing_rank);
    ...
    if (!skip_inertia_check_ && check_NegEVals && (numberOfNegEVals != negevals_)) {
        Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA,
                       "In Ma27TSolverInterface::Factorization: negevals_ = %" IPOPT_INDEX_FORMAT
...
ESymSolverStatus Ma27TSolverInterface::Backsolve(Index nrhs, Number *rhs_vals)
{
...
bool Ma27TSolverInterface::IncreaseQuality()
{
    ...
    Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA, "Increasing pivot tolerance for MA27 from %7.2e ", pivtol_);
