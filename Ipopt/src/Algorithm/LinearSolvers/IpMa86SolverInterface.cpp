...
void Ma86SolverInterface::RegisterOptions(SmartPtr<RegisteredOptions> roptions)
{
    ...
    roptions->AddStringOption3("ma86_scaling", "Controls scaling of matrix", "mc64", "none",
                               "Do not scale the linear system matrix", "mc64", "Scale linear system matrix using MC64",
                               "mc77", "Scale linear system matrix using MC77 [1,3,0]");
    roptions->AddStringOption3("ma86_order", "Controls type of ordering",
#ifdef COINHSL_HAS_METIS
                               "auto",
#else
                               "amd",
#endif
                               "auto", "Try both AMD and MeTiS, pick best", "amd",
                               "Use the HSL_MC68 approximate minimum degree algorithm", "metis",
                               "Use the MeTiS nested dissection algorithm (if available)");
}

void Ma86SolverInterface::SetFunctions(IPOPT_DECL_MA86_DEFAULT_CONTROL(*ma86_default_control),
                                       IPOPT_DECL_MA86_ANALYSE(*ma86_analyse), IPOPT_DECL_MA86_FACTOR(*ma86_factor),
                                       IPOPT_DECL_MA86_FACTOR_SOLVE(*ma86_factor_solve),
                                       IPOPT_DECL_MA86_SOLVE(*ma86_solve), IPOPT_DECL_MA86_FINALISE(*ma86_finalise),
...
bool Ma86SolverInterface::InitializeImpl(const OptionsList &options, const std::string &prefix)
{
...
ESymSolverStatus Ma86SolverInterface::InitializeStructure(Index dim, Index nonzeros, const Index *ia, const Index *ja)
{
...
ESymSolverStatus Ma86SolverInterface::MultiSolve(bool new_matrix, const Index *ia, const Index *ja, Index nrhs,
                                                 Number *rhs_vals, bool check_NegEVals, Index numberOfNegEVals)
{
    ...
    if (new_matrix || pivtol_changed_) {
        ...
        ma86_solve(0, nrhs, ndim_, rhs_vals, order_, &keep_, &control_, &info, NULL);
        if (HaveIpData()) {
            IpData().TimingStats().LinearSystemBackSolve().End();
        }
    }
