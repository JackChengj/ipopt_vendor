...
const char *ma57_err_msg[] = {"Operation successful.\n",
...
Ma57TSolverInterface::Ma57TSolverInterface(SmartPtr<LibraryLoader> hslloader_) :
...
{
    roptions->AddLowerBoundedIntegerOption("ma57_print_level", "Debug printing level for the linear solver MA57", 0, 0,
                                           "0: no printing; 1: Error messages only; 2: Error and warning messages; 3: "
                                           "Error and warning messages and terse monitoring; >=4: All information.");
    ...
    roptions->AddBoundedNumberOption(
        "ma57_pivtolmax", "Maximum pivot tolerance for the linear solver MA57.", 0.0, true, 1.0, true, 1e-4,
        "Ipopt may increase pivtol as high as ma57_pivtolmax to get a more accurate solution to the linear system.");
    roptions->AddLowerBoundedNumberOption(
        "ma57_pre_alloc", "Safety factor for work space memory allocation for the linear solver MA57.", 1., false, 1.05,
    ...
    roptions->AddBoundedIntegerOption("ma57_pivot_order", "Controls pivot order in MA57", 0, 5,
    ...
    roptions->AddBoolOption("ma57_automatic_scaling", "Controls whether to enable automatic scaling in MA57", false,
    ...
    roptions->AddBoundedIntegerOption(
        "ma57_small_pivot_flag", "Handling of small pivots", 0, 1, 0,
...
void Ma57TSolverInterface::SetFunctions(IPOPT_DECL_MA57A(*ma57a), IPOPT_DECL_MA57B(*ma57b), IPOPT_DECL_MA57C(*ma57c),
                                        IPOPT_DECL_MA57E(*ma57e), IPOPT_DECL_MA57I(*ma57i))
{
...
bool Ma57TSolverInterface::InitializeImpl(const OptionsList &options, const std::string &prefix)
{
    if (user_ma57a != NULL) {
    ...
    wd_icntl_[10] =
        ma57_block_size; /* Block size used by Level 3 BLAS in MA57BD - should be a multiple of 8.  Default is 16. */
    wd_icntl_[11] =
        ma57_node_amalgamation; /* Two nodes of the assembly tree are merged only if both involve less than ICNTL(12) eliminations. Default is 16. */
    // CET: 04-29-2010
    ...
    // CET: Added 04-29-2010 at suggestion of Jonathan Hogg of HSL
    wd_icntl_[15] =
        ma57_small_pivot_flag; /* If set to 1, small entries are removed and corresponding pivots are placed at the end
                                  of factorization.  May be useful for highly rank deficient matrices.  Default is 0. */
    // CET: 04-29-2010
...
ESymSolverStatus Ma57TSolverInterface::MultiSolve(bool new_matrix, const Index *airn, const Index *ajcn, Index nrhs,
...
ESymSolverStatus Ma57TSolverInterface::InitializeStructure(Index dim, Index nonzeros, const Index *airn, const Index *ajcn)
{
...
ESymSolverStatus Ma57TSolverInterface::SymbolicFactorization(const Index *airn, const Index *ajcn)
{
    ...
    if (wd_info_[0] < 0) {
        Jnlst().Printf(J_ERROR, J_LINEAR_ALGEBRA, "*** Error from MA57AD *** INFO(0) = %" IPOPT_INDEX_FORMAT "\n",
                       wd_info_[0]);
    }
    ...
    Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA, "Suggested lfact  (*%e):  %" IPOPT_INDEX_FORMAT "\n", ma57_pre_alloc_,
                   wd_lfact_);
    Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA, "Suggested lifact (*%e):  %" IPOPT_INDEX_FORMAT "\n", ma57_pre_alloc_,
                   wd_lifact_);
...
ESymSolverStatus Ma57TSolverInterface::Factorization(const Index * /*airn*/, const Index * /*ajcn*/,
                                                     bool check_NegEVals, Index numberOfNegEVals)
{
    ...
    while (fact_error) {
        ma57b(&n, &ne, a_, wd_fact_, &wd_lfact_, wd_ifact_, &wd_lifact_, &wd_lkeep_, wd_keep_, wd_iwork_, wd_icntl_,
              wd_cntl_, wd_info_, wd_rinfo_);
        ...
        if (wd_info_[0] == 0) {
            ...
            ComputeMemIncrease(wd_lfact_, (Number)wd_info_[16] * ma57_pre_alloc_, (ma57int)0,
                               "double working space for MA57");
            Jnlst().Printf(J_WARNING, J_LINEAR_ALGEBRA,
                           "Reallocating memory for MA57: lfact (%" IPOPT_INDEX_FORMAT ")\n", wd_lfact_);

            // I removed this, because the call to new below should already check this, too, and would throw a
            // std::bad_alloc (which we catch) (or std::bad_array_new_length if C++11) if( (size_t) wd_lfact_ >
            // std::numeric_limits<size_t>::max() / sizeof(Number) )
            // {
            ...
            ma57int idmy;
            ma57e(&n, &ic, wd_keep_, wd_fact_, &wd_info_[1], temp, &wd_lfact_, wd_ifact_, &wd_info_[1], &idmy,
                  &wd_lfact_, wd_info_);
            delete[] wd_fact_;
            wd_fact_ = temp;
            ...
            ComputeMemIncrease(wd_lifact_, (Number)wd_info_[17] * ma57_pre_alloc_, (ma57int)0,
                               "integer working space for MA57");
            temp = new ma57int[wd_lifact_];

            Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA, "Reallocating lifact (%" IPOPT_INDEX_FORMAT ")\n", wd_lifact_);

            Number ddmy;
            ma57e(&n, &ic, wd_keep_, wd_fact_, &wd_info_[1], &ddmy, &wd_lifact_, wd_ifact_, &wd_info_[1], temp,
                  &wd_lifact_, wd_info_);
            delete[] wd_ifact_;
            wd_ifact_ = temp;
        } else if (wd_info_[0] < 0) {
            Jnlst().Printf(J_ERROR, J_LINEAR_ALGEBRA, "Error in MA57BD:  %" IPOPT_INDEX_FORMAT "\n", wd_info_[0]);
        ...
        // Check if the system is singular.
            ...
            Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA, "System singular, rank = %" IPOPT_INDEX_FORMAT "\n", wd_info_[24]);
            return SYMSOLVER_SINGULAR;
        } else if (wd_info_[0] > 0) {
            Jnlst().Printf(J_ERROR, J_LINEAR_ALGEBRA, "Warning in MA57BD:  %" IPOPT_INDEX_FORMAT "\n", wd_info_[0]);
    ...
    Number peak_mem = 1.0e-3 * ((Number)wd_lfact_ * sizeof(Number) + (Number)wd_lifact_ * sizeof(ma57int) +
                                (Number)wd_lkeep_ * sizeof(ma57int));
    Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA, "MA57 peak memory use: %zdKB\n", (size_t)(peak_mem));
    ...
    if (check_NegEVals && (numberOfNegEVals != negevals_)) {
        Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA,
                       "In Ma57TSolverInterface::Factorization: negevals_ = %" IPOPT_INDEX_FORMAT
                       ", but numberOfNegEVals = %" IPOPT_INDEX_FORMAT "\n",
...
ESymSolverStatus Ma57TSolverInterface::Backsolve(Index nrhs, Number *rhs_vals)
{
    ...
    ma57c(&job, &n, wd_fact_, &wd_lfact_, wd_ifact_, &wd_lifact_, &nrhs_X, rhs_vals, &lrhs, work, &lwork, wd_iwork_,
          wd_icntl_, wd_info_);
    if (wd_info_[0] != 0) {
        Jnlst().Printf(J_ERROR, J_LINEAR_ALGEBRA, "Error in MA57CD:  %" IPOPT_INDEX_FORMAT ".\n", wd_info_[0]);
    }
...
bool Ma57TSolverInterface::IncreaseQuality()
{
    ...
    Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA, "Increasing pivot tolerance for MA57 from %7.2e ", pivtol_);
