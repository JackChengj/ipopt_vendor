...
extern "C" {
extern void IPOPT_HSL_FUNCP(dump_mat_csc, DUMP_MAT_CSC)(const ipindex *factidx, const ipindex *n, const ipindex *ptr,
                                                        const ipindex *row, const ipnumber *a);
}
#endif
...
{
    roptions->AddIntegerOption("ma97_print_level", "Debug printing level", -1,
                               "<0: no printing; 0: Error and warning messages only; 1: Limited diagnostic printing; "
                               ">1 Additional diagnostic printing.");
    roptions->AddLowerBoundedIntegerOption(
        "ma97_nemin", "Node Amalgamation parameter", 1, 8,
    ...
    roptions->AddStringOption5(
        "ma97_scaling", "Specifies strategy for scaling", "dynamic", "none", "Do not scale the linear system matrix",
        "mc30", "Scale all linear system matrices using MC30", "mc64", "Scale all linear system matrices using MC64",
        "mc77", "Scale all linear system matrices using MC77 [1,3,0]", "dynamic",
        "Dynamically select scaling according to rules specified by ma97_scalingX and ma97_switchX options.");
    roptions->AddStringOption4("ma97_scaling1", "First scaling.", "mc64", "none", "No scaling", "mc30",
                               "Scale linear system matrix using MC30", "mc64", "Scale linear system matrix using MC64",
    ...
    roptions->AddStringOption9(
        "ma97_switch1", "First switch, determine when ma97_scaling1 is enabled.", "od_hd_reuse", "never",
        "Scaling is never enabled.", "at_start", "Scaling to be used from the very start.", "at_start_reuse",
        ...
        "Scaling to be used only when previous itr created more that 0.05*n additional delays, otherwise reuse scaling "
        "from previous itr",
    ...
    roptions->AddStringOption4("ma97_scaling2", "Second scaling.", "mc64", "none", "No scaling", "mc30",
                               ...
                               true);
    roptions->AddStringOption9(
        "ma97_switch2", "Second switch, determine when ma97_scaling2 is enabled.", "never", "never",
        "Scaling is never enabled.", "at_start", "Scaling to be used from the very start.", "at_start_reuse",
        "Scaling to be used on first iteration, then reused thereafter.", "on_demand",
        "Scaling to be used after Ipopt request improved solution (i.e. iterative refinement has failed).",
        "on_demand_reuse", "As on_demand, but reuse scaling from previous itr", "high_delay",
    ...
    roptions->AddStringOption4(
        "ma97_scaling3", "Third scaling.", "mc64", "none", "No scaling", "mc30", "Scale linear system matrix using MC30",
        "mc64", "Scale linear system matrix using MC64", "mc77", "Scale linear system matrix using MC77 [1,3,0]",
        "If ma97_scaling=dynamic, this scaling is used according to the trigger ma97_switch3.", true);
    roptions->AddStringOption9(
        "ma97_switch3", "Third switch, determine when ma97_scaling3 is enabled.", "never", "never",
        "Scaling is never enabled.", "at_start", "Scaling to be used from the very start.", "at_start_reuse",
        "Scaling to be used on first iteration, then reused thereafter.", "on_demand",
        "Scaling to be used after Ipopt request improved solution (i.e. iterative refinement has failed).",
        "on_demand_reuse", "As on_demand, but reuse scaling from previous itr", "high_delay",
        ...
        "od_hd", "Combination of on_demand and high_delay", "od_hd_reuse",
        "Combination of on_demand_reuse and high_delay_reuse",
        "If ma97_scaling=dynamic, ma97_scaling3 is enabled according to this condition.", true);
    roptions->AddStringOption7(
        "ma97_order", "Controls type of ordering", "auto", "auto",
...
#ifdef MA97_DUMP_MATRIX
    roptions->AddStringOption2("ma97_dump_matrix", "Controls whether HSL_MA97 dumps each matrix to a file", "no", "no",
                               "Do not dump matrix", "yes", "Do dump matrix");
#endif
...
void Ma97SolverInterface::SetFunctions(IPOPT_DECL_MA97_DEFAULT_CONTROL(*ma97_default_control),
                                       IPOPT_DECL_MA97_ANALYSE(*ma97_analyse), IPOPT_DECL_MA97_FACTOR(*ma97_factor),
                                       IPOPT_DECL_MA97_FACTOR_SOLVE(*ma97_factor_solve),
                                       IPOPT_DECL_MA97_SOLVE(*ma97_solve), IPOPT_DECL_MA97_FINALISE(*ma97_finalise),
...
bool Ma97SolverInterface::InitializeImpl(const OptionsList &options, const std::string &prefix)
{
...
ESymSolverStatus Ma97SolverInterface::InitializeStructure(Index dim, Index nonzeros, const Index *ia, const Index *ja)
{
    ...
    if (ordering_ == ORDER_BEST) {
        ...
        Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA, "AMD   nfactor = %ld, nflops = %ld:\n", info2.num_factor,
                       info2.num_flops);
        ...
        Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA, "MeTiS nfactor = %ld, nflops = %ld:\n", info.num_factor,
                       info.num_flops);
    ...
    Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA, "HSL_MA97: PREDICTED nfactor %ld, maxfront %d\n", info.num_factor,
                   info.maxfront);
...
ESymSolverStatus Ma97SolverInterface::MultiSolve(bool new_matrix, const Index *ia, const Index *ja, Index nrhs,
                                                 Number *rhs_vals, bool check_NegEVals, Index numberOfNegEVals)
{
    ...
    if (new_matrix || pivtol_changed_) {
#ifdef MA97_DUMP_MATRIX
        if (dump_) {
            Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA, "Dumping matrix %d\n", fctidx_);
        ...
        Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA, "HSL_MA97: delays %d, nfactor %ld, nflops %ld, maxfront %d\n",
...
bool Ma97SolverInterface::IncreaseQuality()
{
    ...
    Jnlst().Printf(J_DETAILED, J_LINEAR_ALGEBRA, "Increasing pivot tolerance for HSL_MA97 from %7.2e ", control_.u);
