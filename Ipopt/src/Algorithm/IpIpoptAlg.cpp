...
IpoptAlgorithm::IpoptAlgorithm(const SmartPtr<SearchDirectionCalculator> &search_dir_calculator,
                               const SmartPtr<LineSearch> &line_search, const SmartPtr<MuUpdate> &mu_update,
                               const SmartPtr<ConvergenceCheck> &conv_check,
                               const SmartPtr<IterateInitializer> &iterate_initializer,
                               const SmartPtr<IterationOutput> &iter_output, const SmartPtr<HessianUpdater> &hessian_updater,
...
{
    DBG_START_METH("IpoptAlgorithm::IpoptAlgorithm", dbg_verbosity);
...
void IpoptAlgorithm::RegisterOptions(SmartPtr<RegisteredOptions> roptions)
{
    ...
    roptions->AddLowerBoundedNumberOption(
        "kappa_sigma", "Factor limiting the deviation of dual variables from primal estimates.", 0., true, 1e10,
    ...
    roptions->AddLowerBoundedNumberOption("recalc_y_feas_tol",
                                          "Feasibility threshold for recomputation of multipliers.", 0., true, 1e-6,
    ...
    roptions->AddBoolOption(
        "mehrotra_algorithm", "Indicates whether to do Mehrotra's predictor-corrector algorithm.", false,
    ...
    roptions->AddBoolOption("sb", "whether to skip printing Ipopt copyright banner", false);
    roptions->SetRegisteringCategory("Miscellaneous");
    roptions->AddBoolOption("timing_statistics",
                            "Indicates whether to measure time spend in components of Ipopt and NLP evaluation", false,
                            "The overall algorithm time is unaffected by this option.");
}

static bool copyright_message_printed = false;

bool IpoptAlgorithm::InitializeImpl(const OptionsList &options, const std::string &prefix)
{
    DBG_START_METH("IpoptAlgorithm::InitializeImpl", dbg_verbosity);
...
public:
    EndTasks(TimedTask &task, int excepts) : task_(task), excepts_(excepts) { DBG_ASSERT(task_.IsStarted()); }
...
void IpoptAlgorithm::UpdateHessian()
{
    ...
    Jnlst().Printf(J_DETAILED, J_MAIN, "*** Update HessianMatrix for Iteration %" IPOPT_INDEX_FORMAT ":",
                   IpData().iter_count());
    Jnlst().Printf(J_DETAILED, J_MAIN, "\n**************************************************\n\n");
...
{
    Jnlst().Printf(J_DETAILED, J_MAIN, "\n**************************************************\n");
    Jnlst().Printf(J_DETAILED, J_MAIN, "*** Update Barrier Parameter for Iteration %" IPOPT_INDEX_FORMAT ":",
                   IpData().iter_count());
    Jnlst().Printf(J_DETAILED, J_MAIN, "\n**************************************************\n\n");
...
bool IpoptAlgorithm::ComputeSearchDirection()
{
    ...
    Jnlst().Printf(J_DETAILED, J_MAIN, "\n**************************************************\n");
    Jnlst().Printf(J_DETAILED, J_MAIN, "*** Solving the Primal Dual System for Iteration %" IPOPT_INDEX_FORMAT ":",
                   IpData().iter_count());
    Jnlst().Printf(J_DETAILED, J_MAIN, "\n**************************************************\n\n");
    ...
    if (retval) {
        Jnlst().Printf(J_MOREVECTOR, J_MAIN, "*** Step Calculated for Iteration: %" IPOPT_INDEX_FORMAT "\n",
                       IpData().iter_count());
        IpData().delta()->Print(Jnlst(), J_MOREVECTOR, J_MAIN, "delta");
    } else {
        Jnlst().Printf(J_DETAILED, J_MAIN, "*** Step could not be computed in iteration %" IPOPT_INDEX_FORMAT "!\n",
                       IpData().iter_count());
    }
...
{
    Jnlst().Printf(J_DETAILED, J_MAIN, "\n**************************************************\n");
    Jnlst().Printf(J_DETAILED, J_MAIN, "*** Finding Acceptable Trial Point for Iteration %" IPOPT_INDEX_FORMAT ":",
                   IpData().iter_count());
    Jnlst().Printf(J_DETAILED, J_MAIN, "\n**************************************************\n\n");
...
void IpoptAlgorithm::AcceptTrialPoint()
{
    ...
    if (adjusted_slacks > 0) {
        ...
        if (adjusted_slacks == 1) {
            Jnlst().Printf(J_WARNING, J_MAIN,
                           "In iteration %" IPOPT_INDEX_FORMAT ", %" IPOPT_INDEX_FORMAT
                           " Slack too small, adjusting variable bound\n",
                           IpData().iter_count(), adjusted_slacks);
        } else {
            Jnlst().Printf(J_WARNING, J_MAIN,
                           "In iteration %" IPOPT_INDEX_FORMAT ", %" IPOPT_INDEX_FORMAT
                           " Slacks too small, adjusting variable bounds\n",
                           IpData().iter_count(), adjusted_slacks);
        }
    ...
    max_correction =
        correct_bound_multiplier(*IpData().trial()->z_L(), *IpCq().trial_slack_x_L(), *IpCq().trial_compl_x_L(), new_z_L);
    if (max_correction > 0.) {
    ...
    max_correction =
        correct_bound_multiplier(*IpData().trial()->z_U(), *IpCq().trial_slack_x_U(), *IpCq().trial_compl_x_U(), new_z_U);
    if (max_correction > 0.) {
    ...
    max_correction =
        correct_bound_multiplier(*IpData().trial()->v_L(), *IpCq().trial_slack_s_L(), *IpCq().trial_compl_s_L(), new_v_L);
    if (max_correction > 0.) {
    ...
    max_correction =
        correct_bound_multiplier(*IpData().trial()->v_U(), *IpCq().trial_slack_s_U(), *IpCq().trial_compl_s_U(), new_v_U);
    if (max_correction > 0.) {
    ...
    if (recalc_y_ && IpCq().curr_constraint_violation() < recalc_y_feas_tol_) {
        if (Jnlst().ProduceOutput(J_MOREDETAILED, J_MAIN)) {
            Jnlst().Printf(J_MOREDETAILED, J_MAIN, "dual infeasibility before least square multiplier update = %e\n",
...
{
    DBG_START_METH("IpoptAlgorithm::ComputeFeasibilityMultipliers", dbg_verbosity);
    ...
    if (IsNull(eq_multiplier_calculator_)) {
        Jnlst().Printf(J_DETAILED, J_SOLUTION,
                       "No eq_mult_calculator object available in IpoptAlgorithm to recompute multipliers at solution "
                       "for square problem.\n");
        return;
    }
    ...
    Jnlst().Printf(J_DETAILED, J_SOLUTION, "Restoring iterate from before trying eq_mult_calculator.\n");
...
{
    DBG_START_METH("IpoptAlgorithm::ComputeFeasibilityMultipliersPostprocess", dbg_verbosity);
    DBG_ASSERT(IpCq().IsSquareProblem());
...
void IpoptAlgorithm::calc_number_of_bounds(const Vector &x, const Vector &x_L, const Vector &x_U, const Matrix &Px_L,
                                           const Matrix &Px_U, Index &n_tot, Index &n_only_lower, Index &n_both,
                                           Index &n_only_upper)
{
    DBG_START_METH("IpoptAlgorithm::calc_number_of_bounds", dbg_verbosity);
...
Number IpoptAlgorithm::correct_bound_multiplier(const Vector &trial_z, const Vector &trial_slack,
                                                const Vector &trial_compl, SmartPtr<const Vector> &new_trial_z)
{
    DBG_START_METH("IpoptAlgorithm::CorrectBoundMultiplier", dbg_verbosity);
