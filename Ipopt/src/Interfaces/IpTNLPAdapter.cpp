...
TNLPAdapter::TNLPAdapter(const SmartPtr<TNLP> tnlp, const SmartPtr<const Journalist> jnlst /* = NULL */
                         ) :
    tnlp_(tnlp),
    jnlst_(jnlst),
    full_x_(NULL),
...
void TNLPAdapter::RegisterOptions(SmartPtr<RegisteredOptions> roptions)
{
    ...
    roptions->AddStringOption4(
        ...
        "make_parameter", "Remove fixed variable from optimization variables", "make_parameter_nodual",
        "Remove fixed variable from optimization variables and do not compute bound multipliers for fixed variables",
        "make_constraint", "Add equality constraints fixing variables", "relax_bounds", "Relax fixing bound constraints",
        "The main difference between those options is that the starting point in the \"make_constraint\" case still "
        "has the fixed variables at their given values, whereas in the case \"make_parameter(_nodual)\" the functions "
        "are always "
...
#endif
#if ((defined(COINHSL_HAS_MA28) && !defined(IPOPT_SINGLE)) || (defined(COINHSL_HAS_MA28S) && defined(IPOPT_SINGLE))) && \
    defined(F77_FUNC) && !defined(IPOPT_INT64)
    options.push_back("ma28");
    descrs.push_back("use MA28");
#endif
    roptions->AddStringOption(
        "dependency_detector",
        "Indicates which linear solver should be used to detect linearly dependent equality constraints.", "none",
        options, descrs, "This is experimental and does not work well.", true);
    roptions->AddBoolOption("dependency_detection_with_rhs",
                            "Indicates if the right hand sides of the constraints should be considered in addition to "
                            "gradients during dependency detection",
                            false, "", true);
    roptions->AddLowerBoundedIntegerOption(
        "num_linear_variables", "Number of linear variables", 0, 0,
    ...
    roptions->AddStringOption2("jacobian_approximation", "Specifies technique to compute constraint Jacobian", "exact",
                               ...
                               "user-provided structure, values by finite differences", "", true);
    roptions->AddStringOption2("gradient_approximation", "Specifies technique to compute objective Gradient", "exact",
                               "exact", "user-provided gradient", "finite-difference-values",
                               "values by finite differences", "", true);
    roptions->AddLowerBoundedNumberOption(
        "findiff_perturbation", "Size of the finite difference perturbation for derivative approximation.", 0., true,
        1e-7, "This determines the relative perturbation of the variable entries.", true);

    roptions->SetRegisteringCategory("Derivative Checker");
    roptions->AddStringOption4(
        "derivative_test", "Enable derivative checker", "none", "none", "do not perform derivative test", "first-order",
        "perform test of first derivatives at starting point", "second-order",
        "perform test of first and second derivatives at starting point", "only-second-order",
    ...
    roptions->AddLowerBoundedIntegerOption(
        "derivative_test_first_index", "Index of first quantity to be checked by derivative checker", -2, -2,
        "If this is set to -2, then all derivatives are checked. "
        "Otherwise, for the first derivative test it specifies the first variable for which the test is done (counting "
        "starts at 0). "
    ...
    roptions->AddLowerBoundedNumberOption("derivative_test_perturbation",
                                          "Size of the finite difference perturbation in derivative test.", 0., true,
                                          1e-8, "This determines the relative perturbation of the variable entries.");
    roptions->AddLowerBoundedNumberOption(
        "derivative_test_tol", "Threshold for indicating wrong derivative.", 0., true, 1e-4,
    ...
    roptions->AddBoolOption("derivative_test_print_all",
                            "Indicates whether information for all estimated derivatives should be printed.", false,
                            "Determines verbosity of derivative checker.");
    roptions->AddLowerBoundedNumberOption(
        "point_perturbation_radius", "Maximal perturbation of an evaluation point.", 0., false, 10.,
        "If a random perturbation of a points is required, this number indicates the maximal perturbation. "
        "This is for example used when determining the center point at which the finite difference derivative test is "
        "executed.");
}

bool TNLPAdapter::ProcessOptions(const OptionsList &options, const std::string &prefix)
{
    ...
    if (dependency_detector == "mumps") {
        ...
        SmartPtr<TSymLinearSolver> ScaledSolver = new TSymLinearSolver(SolverInterface, NULL);
...
bool TNLPAdapter::GetSpaces(SmartPtr<const VectorSpace> &x_space, SmartPtr<const VectorSpace> &c_space,
                            SmartPtr<const VectorSpace> &d_space, SmartPtr<const VectorSpace> &x_l_space,
                            SmartPtr<const MatrixSpace> &px_l_space, SmartPtr<const VectorSpace> &x_u_space,
                            SmartPtr<const MatrixSpace> &px_u_space, SmartPtr<const VectorSpace> &d_l_space,
                            SmartPtr<const MatrixSpace> &pd_l_space, SmartPtr<const VectorSpace> &d_u_space,
                            SmartPtr<const MatrixSpace> &pd_u_space, SmartPtr<const MatrixSpace> &Jac_c_space,
                            SmartPtr<const MatrixSpace> &Jac_d_space, SmartPtr<const SymMatrixSpace> &Hess_lagrangian_space)
{
    ...
    ASSERT_EXCEPTION(!warm_start_same_structure_ || (n_full_x == n_full_x_ && n_full_g == n_full_g_ &&
                                                     nz_full_jac_g == nz_full_jac_g_ && nz_full_h == nz_full_h_),
                     INVALID_WARMSTART, "warm_start_same_structure chosen, but problem dimensions are different.");
    n_full_x_ = n_full_x;
    n_full_g_ = n_full_g;
    ...
    if (!warm_start_same_structure_) {
        ...
        while (!done) {
            ...
            for (Index i = 0; i < n_full_x_; i++) {
                ...
                if (lower_bound == upper_bound) {
                    ...
                    char string[128];
                    Snprintf(string, 127,
                             "There are inconsistent bounds on variable %" IPOPT_INDEX_FORMAT
                             ": lower = %25.16e and upper = %25.16e.",
            ...
            for (Index i = 0; i < n_full_g_; i++) {
                ...
                if (lower_bound == upper_bound) {
                    ...
                    char string[128];
                    Snprintf(string, 127,
                             "There are inconsistent bounds on constraint function %" IPOPT_INDEX_FORMAT
                             ": lower = %25.16e and upper = %25.16e.",
                             i, lower_bound, upper_bound);
                    THROW_EXCEPTION(INCONSISTENT_BOUNDS, string);
                } else {
        ...
        if (n_c > 0 && IsValid(dependency_detector_)) {
            ...
            if (jnlst_->ProduceOutput(J_DETAILED, J_INITIALIZATION)) {
                ...
                for (std::list<Index>::iterator i = c_deps.begin(); i != c_deps.end(); ++i) {
                    jnlst_->Printf(J_DETAILED, J_INITIALIZATION,
                                   "c_dep[%" IPOPT_INDEX_FORMAT "] = %" IPOPT_INDEX_FORMAT "\n", count++, *i);
                }
                jnlst_->Printf(J_DETAILED, J_INITIALIZATION, "\n");
            }
            if (!c_deps.empty()) {
        ...
        if (n_x_fixed_ > 0 &&
            (fixed_variable_treatment_ == MAKE_PARAMETER || fixed_variable_treatment_ == MAKE_PARAMETER_NODUAL)) {
        ...
        if (hessian_approximation_ == EXACT) {
            ...
            retval = tnlp_->eval_h(n_full_x_, NULL, false, 0, n_full_g_, NULL, false, nz_full_h_, full_h_iRow,
                                   full_h_jCol, NULL);
            if (!retval) {
                delete[] full_h_iRow;
                delete[] full_h_jCol;
                ...
                jnlst_->Printf(J_ERROR, J_INITIALIZATION,
                               "Option \"hessian_approximation\" is not chosen as \"limited-memory\", but eval_h "
                               "returns false.\n");
    ...
    if (IsValid(jnlst_)) {
        jnlst_->Printf(J_ITERSUMMARY, J_STATISTICS,
                       "Number of nonzeros in equality constraint Jacobian...:%9" IPOPT_INDEX_FORMAT "\n", nz_jac_c_);
        jnlst_->Printf(J_ITERSUMMARY, J_STATISTICS,
                       "Number of nonzeros in inequality constraint Jacobian.:%9" IPOPT_INDEX_FORMAT "\n", nz_jac_d_);
        jnlst_->Printf(J_ITERSUMMARY, J_STATISTICS,
                       "Number of nonzeros in Lagrangian Hessian.............:%9" IPOPT_INDEX_FORMAT "\n\n", nz_h_);
    }

    return true;
}

bool TNLPAdapter::GetBoundsInformation(const Matrix &Px_L, Vector &x_L, const Matrix &Px_U, Vector &x_U,
                                       const Matrix &Pd_L, Vector &d_L, const Matrix &Pd_U, Vector &d_U)
{
...
bool TNLPAdapter::GetStartingPoint(SmartPtr<Vector> x, bool need_x, SmartPtr<Vector> y_c, bool need_y_c,
                                   SmartPtr<Vector> y_d, bool need_y_d, SmartPtr<Vector> z_L, bool need_z_L,
                                   SmartPtr<Vector> z_U, bool need_z_U)
{
...
bool TNLPAdapter::Eval_h(const Vector &x, Number obj_factor, const Vector &yc, const Vector &yd, SymMatrix &h)
{
...
void TNLPAdapter::GetScalingParameters(const SmartPtr<const VectorSpace> x_space, const SmartPtr<const VectorSpace> c_space,
                                       const SmartPtr<const VectorSpace> d_space, Number &obj_scaling,
                                       SmartPtr<Vector> &x_scaling, SmartPtr<Vector> &c_scaling,
                                       SmartPtr<Vector> &d_scaling) const
{
    x_scaling = x_space->MakeNew();
    c_scaling = c_space->MakeNew();
    d_scaling = d_space->MakeNew();
    DBG_ASSERT(c_scaling->Dim() + d_scaling->Dim() ==
               n_full_g_ + (fixed_variable_treatment_ == MAKE_CONSTRAINT ? n_x_fixed_ : 0));
    ...
    if (IsValid(P_x_full_x_)) {
        ...
        bool retval = tnlp_->get_scaling_parameters(obj_scaling, use_x_scaling, n_full_x_, full_x_scaling,
                                                    use_g_scaling, n_full_g_, full_g_scaling);
        if (!retval) {
            delete[] full_x_scaling;
            jnlst_->Printf(J_ERROR, J_INITIALIZATION,
                           "Option nlp_scaling_method selected as user-scaling, but no user-scaling available, or it "
                           "cannot be computed.\n");
...
void TNLPAdapter::FinalizeSolution(SolverReturn status, const Vector &x, const Vector &z_L, const Vector &z_U,
                                   const Vector &c, const Vector &d, const Vector &y_c, const Vector &y_d,
    ...
    for (y_c_meta_iter = y_c_meta.begin(); y_c_meta_iter != y_c_meta.end(); ++y_c_meta_iter) {
        if ((Index)y_c_meta_iter->second.size() == y_c.Dim()) {
            if (y_d_space->HasNumericMetaData(y_c_meta_iter->first.c_str())  // There exists a corresponding y_d metadata
                && ((Index)(y_d_meta.find(y_c_meta_iter->first)->second).size()) ==
                       y_d.Dim())  // and y_d metadata vector has size y_d.Dim()
            {
    ...
    switch (status) {
        ...
        case ERROR_IN_STEP_COMPUTATION:
            ...
            if (!ResortBoundMultipliers(x, y_c, y_d, z_L, full_z_L, z_U, full_z_U)) {
                jnlst_->Printf(J_WARNING, J_INITIALIZATION,
                               "Failed to evaluate gradient of objective or constraints when computing bound "
...
bool TNLPAdapter::IntermediateCallBack(AlgorithmMode mode, Index iter, Number obj_value, Number inf_pr, Number inf_du,
...
{
    return tnlp_->intermediate_callback(mode, iter, obj_value, inf_pr, inf_du, mu, d_norm, regularization_size,
                                        alpha_du, alpha_pr, ls_trials, ip_data, ip_cq);
}

void TNLPAdapter::GetQuasiNewtonApproximationSpaces(SmartPtr<VectorSpace> &approx_space, SmartPtr<Matrix> &P_approx)
{
    ...
    if (num_nonlin_vars < 0) {
        ...
        if (!retval) {
            ...
            jnlst_->Printf(J_ERROR, J_INITIALIZATION,
                           "TNLP's get_number_of_nonlinear_variables returns non-negative number, but "
                           "get_list_of_nonlinear_variables returns false.\n");
    ...
    if (IsNull(P_x_full_x_)) {
        ...
        if (nonfixed_nonlin_vars == n_x_free) {
            ...
            SmartPtr<ExpansionMatrixSpace> ex_sp =
                new ExpansionMatrixSpace(n_x_free, nonfixed_nonlin_vars, nonfixed_pos_nonlin_vars);
            P_approx = ex_sp->MakeNew();
...
void TNLPAdapter::ResortX(const Vector &x, Number *x_orig, bool usefixedvals)
{
...
void TNLPAdapter::ResortG(const Vector &c, const Vector &d, Number *g_orig, bool correctrhs)
{
...
void TNLPAdapter::ResortBounds(const Vector &x_L, Number *x_L_orig, const Vector &x_U, Number *x_U_orig)
{
    if (x_L_orig) {
...
bool TNLPAdapter::ResortBoundMultipliers(const Vector &x, const Vector &y_c, const Vector &y_d, const Vector &z_L,
                                         Number *z_L_orig, const Vector &z_U, Number *z_U_orig)
{
...
void TNLPAdapter::initialize_findiff_jac(const Index *iRow, const Index *jCol)
{
    ...
    if (findiff_jac_nnz_ != nz_full_jac_g_) {
        THROW_EXCEPTION(INVALID_TNLP,
                        "Sparsity structure of Jacobian has multiple occurrences of the same position.  This is not "
                        "allowed for finite differences.");
    }
...
bool TNLPAdapter::CheckDerivatives(TNLPAdapter::DerivativeTestEnum deriv_test, Index deriv_test_start_index)
{
    ...
    ASSERT_EXCEPTION(IsValid(jnlst_), ERROR_IN_TNLP_DERIVATIVE_TEST,
                     "No Journalist given to TNLPAdapter.  Need Journalist, otherwise can't produce any output in "
                     "DerivativeChecker!");
        ...
        for (Index ivar = ivar_first; ivar < nx; ivar++) {
            ...
            if (cflag != ' ' || derivative_test_print_all_) {
                jnlst_->Printf(J_WARNING, J_NLP,
                               "%c grad_f[      %5" IPOPT_INDEX_FORMAT "] = %23.16e    ~ %23.16e  [%10.3e]\n", cflag,
            ...
            if (ng > 0) {
                ...
                for (Index icon = 0; icon < ng; icon++) {
                    ...
                    if (cflag != ' ' || derivative_test_print_all_) {
                        jnlst_->Printf(J_WARNING, J_NLP,
                                       "%c jac_g [%5" IPOPT_INDEX_FORMAT ",%5" IPOPT_INDEX_FORMAT
                                       "] = %23.16e %c  ~ %23.16e  [%10.3e]\n",
                                       cflag, icon + index_correction, ivar + index_correction, deriv_exact, sflag,
        ...
        for (Index icon = icon_first; icon < ng; icon++) {
            ...
            for (Index ivar = 0; ivar < nx; ivar++) {
                ...
                if (icon == -1) {
                    ...
                    ASSERT_EXCEPTION(
                        retval, ERROR_IN_TNLP_DERIVATIVE_TEST,
                        "In TNLP derivative test: Jacobian values could not be evaluated at reference point.");
                ...
                for (Index ivar2 = 0; ivar2 < nx; ivar2++) {
                    ...
                    Number rel_error =
                        std::abs(deriv_approx - deriv_exact) / Max(std::abs(deriv_approx), derivative_test_tol_);
                    char cflag = ' ';
                    if (rel_error >= derivative_test_tol_) {
                        cflag = '*';
                        nerrors++;
                    ...
                    if (cflag != ' ' || derivative_test_print_all_) {
                        if (icon == -1) {
                            jnlst_->Printf(J_WARNING, J_NLP,
                                           "%c             obj_hess[%5" IPOPT_INDEX_FORMAT ",%5" IPOPT_INDEX_FORMAT
                                           "] = %23.16e %c  ~ %23.16e  [%10.3e]\n",
                                           cflag, ivar + index_correction, ivar2 + index_correction, deriv_exact, sflag,
                                           deriv_approx, rel_error);
                        } else {
                            jnlst_->Printf(J_WARNING, J_NLP,
                                           "%c %5" IPOPT_INDEX_FORMAT "-th constr_hess[%5" IPOPT_INDEX_FORMAT
                                           ",%5" IPOPT_INDEX_FORMAT "] = %23.16e %c  ~ %23.16e  [%10.3e]\n",
                                           cflag, icon + index_correction, ivar + index_correction,
                                           ivar2 + index_correction, deriv_exact, sflag, deriv_approx, rel_error);
                        }
                    }
                }
...
bool TNLPAdapter::DetermineDependentConstraints(Index n_x_var, const Index *x_not_fixed_map, const Number *x_l,
                                                const Number *x_u, const Number *g_l, const Number * /*g_u*/, Index n_c,
                                                const Index *c_map, std::list<Index> &c_deps)
{
    ...
    if (!tnlp_->get_starting_point(n_full_x_, true, full_x_, false, NULL, NULL, n_full_g_, false, NULL)) {
        delete[] jac_c_iRow;
        delete[] jac_c_jCol;
        delete[] jac_c_map;
        return false;
    ...
    bool retval =
        dependency_detector_->DetermineDependentRows(n_c, n_x_var, nz_jac_c, jac_c_vals, jac_c_iRow, jac_c_jCol, c_deps);
